...
cd /cs/research/medim/projects2/projects/frullan/Documents/HighFreqCode/ExperimentsThesis/Ex20_ROI
clear all;
close all;
...
h = plot_projection_compact(u0, param);
a = axes;
a.Visible = 'off'; 
saveas(gcf, './figures/ET20_initialPressure.fig');
saveas(gcf, './figures/ET20_initialPressure', 'png');
...
h = plot_projection_compact(u_roi, param);
a = axes;
a.Visible = 'off'; 
saveas(gcf, './figures/ET20_initialPressure_ROI.fig');
saveas(gcf, './figures/ET20_initialPressure_ROI', 'png');
...
factor = 1e3;
f = figure;
plot3(factor*scatter_points(:, 2), factor*scatter_points(:, 3), factor*scatter_points(:, 1), 'or');
ax = gca;
ax.GridAlpha = 1;
grid on;
axis([0 factor*Ny*dy 0 factor*Nz*dz 0 factor*Nx*dx]);
daspect([1 1 1]);
f.CurrentAxes.ZDir = 'Reverse';
hold on;
colours = winter(single_nRays);
for n = 1:5:single_nRays
    plot3(factor*single_yCoord(:, n), factor*single_zCoord(:, n), factor*single_xCoord(:, n), 'Color', colours(n, :));
end
xlabel('y [mm]');
ylabel('z [mm]');
zlabel('x [mm]');
set(gca, 'FontSize', 15);
saveas(gcf, './figures/ET20_beam', 'png');
...
param.dx = dx;
param.cb = false;
pressureAdj_Matrix = importdata('./output_data/pressureAdjoint_ROI.dat', ' ', 0);
pressureAdj = matrix2cube(pressureAdj_Matrix, Nz);
h = plot_projection_compact(pressureAdj, param);
a = axes;
a.Visible = 'off'; 
saveas(gcf, './figures/ET20_adjoint.fig')
saveas(gcf, './figures/ET20_adjoint', 'png')

pressureAdj_roi = select_roi(pressureAdj, dx, dy, dz, radius, xpos, ypos, zpos);
h = plot_projection_compact(pressureAdj_roi, param);
a = axes;
a.Visible = 'off'; 
saveas(gcf, './figures/ET20_adjoint_ROI.fig')
saveas(gcf, './figures/ET20_adjoint_ROI', 'png')
